...
export const NOTIFICATION_TEMPLATES = {
  ...
  "platform.usage_warning": {
    ...
    priority: "high",
    roles: ["admin"],
    title: (ctx) => `Usage limit warning — ${ctx.metric || "resource"}`,
    message: (ctx) =>
      `${ctx.metric || "Usage"} at ${ctx.percent || 80}% (${ctx.used ?? "—"} / ${ctx.limit ?? "—"}).`,
    route: () => buildRoute("/platform/control-center"),
    actions: () => [linkAction("view", "Open control center", "/platform/control-center")],
  },
  "platform.payment_success": {
    type: "success",
    category: "finance",
    priority: "high",
    roles: ["admin"],
    title: (ctx) => `Platform payment received — ${ctx.tier || "plan"}`,
    message: (ctx) => `INR ${ctx.amountInr ?? "—"} subscription payment confirmed.`,
    route: () => buildRoute("/platform/control-center"),
    actions: () => [linkAction("view", "Billing", "/platform/control-center")],
  },
  "platform.payment_failed": {
    ...
    priority: "urgent",
    roles: ["admin"],
    title: (ctx) => `Platform payment failed — ${ctx.tier || "plan"}`,
    message: (ctx) => `Subscription payment failed for ${ctx.tier || "plan"}.`,
    route: () => buildRoute("/platform/control-center"),
    actions: () => [linkAction("view", "Retry billing", "/platform/control-center")],
  },
  "platform.subscription_cancelled": {
    ...
    priority: "medium",
    roles: ["admin"],
    title: () => "Platform subscription cancelled",
    message: () => "Your platform plan was cancelled and downgraded to Free.",
    route: () => buildRoute("/platform/control-center"),
    actions: () => [linkAction("view", "Manage plan", "/platform/control-center")],
  },
  "platform.maintenance_enabled": {
    type: "warning",
    category: "system",
    priority: "high",
    roles: ["admin", "scanner", "finance"],
    title: () => "Maintenance mode enabled",
    message: (ctx) => ctx.message || "Public access is temporarily paused.",
    route: () => buildRoute("/platform/control-center"),
    actions: () => [linkAction("view", "Control center", "/platform/control-center")],
  },
};
